...
In this Chapter, we present a formalism that shows that interferometers may be sensitive to the global signal. Our analysis is complimentary to the one presented by \cite{Presley.15}, but approaches the problem from a more spherical basis. We also present measurements that are suggestive, but not conclusive, of measurements of the global signal of Galactic foregrounds with the PAPER interferometer.
...
\caption[Comparison of time-averaged PAPER-128 visibilities to the simulated transfer function. ]{Comparison of time-averaged, LST-binned PAPER-128 visibilities to the simulated transfer function in the `ee' polarization. Upper and lower panels show the spectra in frequency and delay space. See the text for discussion.}
\label{fig:global_signal_data_vs_sim}
\end{figure}

The imperfect alignment of the oscillatory spectra was most likely due to some frequency-dependent phase slope, possibly from an imperfect projection of redundant calibration degeneracies \citep[e.g.][Chapter~\ref{chapter:polcal}]{Dillon.17}. Dividing one by the other would, according to Equation~\ref{eq:xi_def}, grant the global spectrum of the Galactic foregrounds, but that requires perfect alignment in order to avoid divisions by zero. \cite{Presley.15} provided an optimal quadratic estimator formalism to address this problem, which may be a promising route to pursue in future investigations.
