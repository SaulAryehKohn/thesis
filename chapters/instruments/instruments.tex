...
While the PAPER-8 array was under operation in Green Bank it became clear that the radio frequency interference (RFI) environment was bad enough to inhibit the science goals of the experiment. ({\color{red} check with James about history of move to SA}).
...
Electronics next to the dipole element amplified the voltage by a factor of $10^6$, which then propagated down a 75\,$\Omega$ coaxial cable. All cables were of the same length to minimize the amount of extra calibration required per feed, and were above-ground.

{\color{red} SOMETHING SOMETHING receiverators}... 

Within an RFI shielded enclosure\footnote{For PAPER and the HERA-19 commissioning array, this enclosure was a specialized shipping container next to the array. For future HERA build-outs, processing will occur in the Karro Array Processing Building (KAPB) and the receiverator architecture will be replaced with underground ``nodes" (see \citet{deBoer.17} for more detail).} the voltage signals were re-amplified by a factor of $10^4$ (to correct for signal loss along the coaxial cables) and an analog bandpass filter was applied. The filter was designed to have a smooth frequency response which was relatively flat between 120 and 180 MHz \citep{MooreThesis}.
...
\caption{PLACEHOLDER: A diagram of the PAPER signal chain.}
...
PAPER and HERA were not the only instruments researching the EoR and how to detect it. Several low-frequency interferometers around the world are contributing to the understanding of the EoR and the difficulties of observing it. Below I briefly describe a few of the leaders of the field.
...
SKA-Low will consist of over 100,000 receiving elements in an imaging configuration. It will be the most powerful low-frequency radio telescope ever created, and be used not only for EoR science but a host of low-frequency science objectives \citep[e.g.][]{SKABook, Schilizzi.07, Dewdney.09}.
